...
module SmartAnswer
  class StartPageContentItemTest < ActiveSupport::TestCase
    ...
    test 'stores external_related_links in the details hash' do
      presenter = stub_flow_registration_presenter
    ...
    test 'stores the introductory paragraph in the details hash as govspeak' do
      presenter = stub_flow_registration_presenter
    ...
    test 'stores more information in the details hash as govspeak' do
      presenter = stub_flow_registration_presenter
    ...
    test 'stores the link to the flow in the details hash' do
      presenter = stub_flow_registration_presenter
      content_item = StartPageContentItem.new(presenter)
    ...
    test 'payload contains the text to be used for the start button' do
      presenter = stub_flow_registration_presenter
    ...
    test 'sets the public_updated_at timestamp to now' do
      now = Time.now
    ...
    test 'registers an exact route using the slug of the smart answer' do
      presenter = stub_flow_registration_presenter
      content_item = StartPageContentItem.new(presenter)
    ...
    test 'registers an exact json route using the slug of the smart answer' do
      presenter = stub_flow_registration_presenter
      content_item = StartPageContentItem.new(presenter)
